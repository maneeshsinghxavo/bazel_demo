...
def _impl(ctx):
    print(ctx.attr.link_flags)
    print(ctx.attr.link_libs)
    #compiler_flags = ctx.attr.compile_flags
    #print(compiler_flags)

    #is_nostd_presnt = [compiler_flag for compiler_flag in ctx.attr.compile_flags if "-nostdinc" in compiler_flag]

    #if  is_nostd_presnt:
    #fail(" -nostdinc option is not allowed in compiler flags")
    ...
    tool_paths = [
        ...
        if is_certified and name != "qcc" or name != "q++"
    ]

    if is_certified:
        tool_paths.append(tool_path(name = "test1", path = "test1"))
    else:
        tool_paths.append(tool_path(name = "test2", path = "test2"))
